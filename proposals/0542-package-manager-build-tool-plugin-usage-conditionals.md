...
This proposal adds conditional use of build tool plugins ([SE-0303](https://github.com/swiftlang/swift-evolution/blob/main/proposals/0303-swiftpm-extensible-build-tools.md)) declared in a target's `plugins:` parameter. It reuses `TargetDependencyCondition` and adds an optional `hostPlatforms` filter. Plugin usages can combine this filter with target-platform and trait filters. All target dependencies can also use the host filter.

Command plugins ([SE-0332](https://github.com/swiftlang/swift-evolution/blob/main/proposals/0332-swiftpm-command-plugins.md)) are package commands that users run directly. They are not declared in a target's `plugins:` parameter. A command plugin can run for the package or for one or more targets. Thus, a target-platform condition does not always have a clear meaning. Command-plugin conditions also need rules for SwiftPM and IDEs. These rules include whether unavailable commands appear in command lists and what happens when a user tries to run one. This proposal does not define command-plugin conditions.

Here, a **build tool plugin** is a sandboxed Swift script that returns `Command` values. A `Command` tells the build system how to invoke an executable before or during a build. It includes arguments and environment variables. A build command can declare input and output files. A prebuild command declares an output directory instead. The executable is the **build tool**. Platform constraints apply to build tools, including generators and other executables invoked during a build. A host condition prevents commands that invoke a build tool on an unsupported host. A target-platform condition filters the commands and their outputs without giving the target platform to the plugin.
...
The `SwiftLintPlugins` package contains a build tool plugin and a command plugin. This example uses `SwiftLintBuildToolPlugin`, not the command plugin. Its commands invoke the SwiftLint binary, which is the build tool.
...
This proposal is additive. Existing plugin usage declarations without a `condition` parameter continue to work as before. Existing target dependency conditions without `hostPlatforms` also keep their current behavior. The new API requires tools version 6.5.
